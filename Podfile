...
# Podfile 采用Ruby语言书写，对大小写敏感，且需要调用的函数（方法）必须先从上而下进行优先申明

##设置workspace文件
workspace 'XCProjManager.xcworkspace'# ❤️❤️❤️
##下面两行是指明依赖库的来源地址
...
proj = 'JobsBaseConfig'
target proj do
  project 'JobsBaseConfig/JobsBaseConfig.xcodeproj'
  # Pods for JobsBaseConfig
  testApp # 单元测试框架
  ...
  func # 一些功能性
  database # 数据库
  # end Pods for JobsBaseConfig
end

proj = 'TargetProj'# ❤️❤️❤️
target proj do
  project 'TargetProj/TargetProj.xcodeproj'# ❤️❤️❤️
  # Pods for TargetProj
  testApp # 单元测试框架
  longConnection # 长链接框架
  debugPods # 调试框架
